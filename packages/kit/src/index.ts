...
const marketCycle = async (
  ...
  for (const market of markets) {
    try {
      ...
      if (market.status === 'pending') {
        ...
        const mint = await createMarket(connection, market, agentKeypair, vaultCreator)
        market.mint = mint
        market.status = 'active'
        market.createdAt = Date.now()
        dirty = true

        log('info', `CREATED | ${market.id} — mint=${mint.slice(0, 12)}... | seed=${sol(market.initialLiquidityLamports)} SOL`)
        continue
      }

      // --- snapshot active markets ---
      if (market.status === 'active') {
        const snapshot = await snapshotMarket(connection, market)
        if (snapshot) {
          log(
            'debug',
            `SNAPSHOT | ${market.id} — price=${snapshot.price.toFixed(6)} SOL | ` +
        ...
        if (Date.now() >= market.deadline * 1000) {
          ...
          const outcome = await resolveMarket(market)
  ...
  if (dirty) {
    saveMarkets(marketsPath, markets)
